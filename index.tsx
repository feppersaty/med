...
interface AppContentDetail {
    [key: string]: string; // e.g., clinica: '...', fisiopatologia: '...'
}
...
interface AppContentItem {
    ...
    name: string;
    viewed: boolean;
    details: AppContentDetail;
    severity?: string; // Specific to Patologías
    mortality?: number; // Specific to Patologías
}

const patologiasData: AppContentItem[] = [
...
const patologiasSeverityColors: { [key: string]: string } = { 'Crítico': 'bg-red-500', 'Emergencia': 'bg-orange-500', 'Urgencia': 'bg-amber-500', 'Grave': 'bg-yellow-500', 'Leve': 'bg-sky-500' };
const patologiasIconMap: { [key: string]: string } = { clinica: '🩺', fisiopatologia: '🔬', diagnostico: '💻', tratamiento: '💊', pronostico: '📈' };

const clinicaQuirurgicaData: AppContentItem[] = [
    {
...
const cqIconMap: { [key: string]: string } = { definicion: '📖', clasificacion: '📊', enfoque_diagnostico: '🔍', manejo_inicial: '🚑', fases_cicatrizacion: '🔄', factores_que_afectan: '⚠️', tipos_de_cicatrizacion: '🩹', cicatrizacion_patologica: '❗'};
...
// const cqChartCanvas = document.getElementById('cq-chart') as HTMLCanvasElement | null; // If CQ gets a chart

let currentActiveModule: 'patologias' | 'clinica' | null = null;
let activeData: AppContentItem[] = [];
let activeSortedData: AppContentItem[] = [];
...
function renderSubjectNavigation(
    ...
    navContainer.querySelectorAll('.nav-item').forEach(navLink => {
        navLink.addEventListener('click', (e) => {
            e.preventDefault();
            const itemId = (navLink as HTMLElement).getAttribute('data-id');
            if (itemId) {
...
function handleNavItemClick(itemId: string, moduleName: 'patologias' | 'clinica') {
    const data = moduleName === 'patologias' ? patologiasData : clinicaQuirurgicaData;
    const sortedData = moduleName === 'patologias' ? sortedPatologiasData : sortedClinicaQuirurgicaData;
...
function addMobileMenuToggleListener(
    ...
    toggleButton.addEventListener('click', () => {
        ...
        (menuArrow as HTMLElement).style.transform = navIsHidden ? 'rotate(0deg)' : 'rotate(180deg)';
    });
}
